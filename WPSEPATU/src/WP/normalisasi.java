...
private void Vektor() {
    try {
        ...
        String sqlQuery = "SELECT a.id, a.nama_sepatu, " +
                "ROUND(EXP(LOG(a.harga) * (SELECT n.bobot FROM normalisasi n WHERE n.id = 1) + " +
                "LOG(a.desain) * (SELECT n.bobot FROM normalisasi n WHERE n.id = 2) + " +
                "LOG(a.kualitas) * (SELECT n.bobot FROM normalisasi n WHERE n.id = 3) + " +
                "LOG(a.kenyaman) * (SELECT n.bobot FROM normalisasi n WHERE n.id = 4) + " +
                "LOG(a.ukuran) * (SELECT n.bobot FROM normalisasi n WHERE n.id = 5)), 3) AS score " +
                "FROM alternatif a;";
        ...
        String insertSql = "INSERT INTO vektor (id, nama_sepatu, score) VALUES (?, ?, ?)";
        ...
        while (rs.next()) {
            ...
            model.addRow(new Object[]{id, nama_sepatu, bobot});
