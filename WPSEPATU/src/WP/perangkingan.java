...
private void tampilkanPerangkingan() {
    try {
        ...
        String sqlQuery = "SELECT vs.id, vs.nama_sepatu, " +
        ...
        DefaultTableModel model = new DefaultTableModel(new String[]{"id", "nama_sepatu", "preferensi"}, 0);

        // Query untuk memasukkan data ke tabel hasil
        String insertSql = "INSERT INTO hasil (id, nama_sepatu, preferensi) VALUES (?, ?, ?)";
        ...
        while (rs.next()) {
            int id = rs.getInt("id");
            String nama_sepatu = rs.getString("nama_sepatu");
            double normalizedScore = rs.getDouble("normalized_score");

            // Tambahkan ke JTable model
            model.addRow(new Object[]{id, nama_sepatu, normalizedScore});

            // Masukkan ke tabel hasil
            insertPst.setInt(1, id);
            insertPst.setString(2, nama_sepatu);
